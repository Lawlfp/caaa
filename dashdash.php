						...
						<div class="con"><div class="darkgreypart">
										...
										if ($result->num_rows > 0) {
											while($row = $result->fetch_assoc()) {
												...
												echo '<a href ="patientrec.php?id='.$row["id"].'"><span class="nottitle" style="width:122px;">Profile</span></a>';
												echo "</div>";
											}
															// Pagination
                                                                $total_pages_sql = "SELECT COUNT(*) FROM patients WHERE id=$doctorid;
                                                                WHERE `id` LIKE '%$search%'       OR `firstName` LIKE '%$search%'		OR `middleName` LIKE '%$search%'		OR `lastName` LIKE '%$search%'        OR `contactNumber` LIKE '%$search%'        OR `doctorid` LIKE '%$search%' 		 OR `lastDiagnosis` LIKE '%$search%'
